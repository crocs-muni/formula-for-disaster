...
@click.command()
@click.option("-f", "--formula", "formula_file", type=click.Path(exists=True,file_okay=True,dir_okay=False), required=True)
@click.option("-p", "prime", type=lambda x: int(x, 0), required=True)
@click.option("-a", "a", type=lambda x: int(x, 0), required=True)
@click.option("-b", "b", type=lambda x: int(x, 0), required=True)
@click.option("-v", "--verbose", is_flag=True)
@click.option("-opd", is_flag=True)
@click.argument("c", type=lambda x: int(x, 0))
def main(formula_file, prime, a, b, verbose, opd, c):
    formula_file = Path(formula_file)
    ZVP = ZVPFinder(formula_file, multiple=c, verbose=verbose, one_point_dependent=opd)
    points = ZVP.find_points(a, b, prime, verbose=verbose)
    print(points)
