...
def fullorder_generator(P : Point, params):
    ...
    if params.curve.model.shortname == 'twisted' or params.curve.model.shortname == 'edwards':
        ...
        if params.curve.model.shortname == 'twisted':
            ...
            invsqrt_a = sqrt_a.inverse()
            P4_raw = { 'x': invsqrt_a, 'y': Mod(0, params.curve.prime)}                        # Affine representation of one 4-order point (its negative is (-1/sqrt(a),0))

        else:
            P4_raw = { 'x': params.curve.parameters['c'], 'y': Mod(0, params.curve.prime)}     # Affine representation of one 4-order point (its negative is (-c,0))
...
def test_add_formula(formula, params):
    ...
    for i in range(1, params.order):
        ...
        for j in range(1, params.order):
            ...
            try:
                ...
                # There is no need for adding points (we have computed by the precomputation step)
                #affine_expected = points[(i + j - 1) % params.order]
                assert(affine_expected == points[(i + j - 1) % params.order])
    ...
    return {\
            ...
            'P + P' : { 'Are there failed computations?' : not unified, 
                'Failures' : unified_counterexample if len(unified_counterexample) < (params.order - 1) else "All the points (not including the point at infinity)", 
                'Number of failures' : len(unified_counterexample) },\
            'P + infty and infty + P' : { 'Are there failed computations?' : not complete, 
                'Failures' : complete_counterexample if len(complete_counterexample) < (params.order - 1) else "All the points (not including the point at infinity)", 
...
def test_dbl_formula(formula, params):
    # Kummer-line has elements in the projective space P^2
    assert( len(params.curve.coordinate_model.variables) > 2 )
    affine_gen = params.generator.to_affine()
    ...
    return { '[2]P' : { 'Are there failed computations?' : not ok, 
        'Failures' : ok_counterexample if len(ok_counterexample) < (params.order - 1) else "All the points (not including the point at infinity)", 
        'Number of failures' : len(ok_counterexample) } }
...
def test_dadd_formula(formula, params):
    # Kummer-line has elements in the projective space P^2
    assert( len(params.curve.coordinate_model.variables) == 2)
...
def test_ladd_formula(formula, params):
    # Kummer-line has elements in the projective space P^2
    assert( len(params.curve.coordinate_model.variables) == 2 )
    affine_gen = kummer_to_affine(params.generator, params)
    ...
    for i in range(1, params.order):
        ...
        for j in range(1, params.order):
            ...
            # There is no need for adding points (we have computed by the precomputation step)
            #affine_expected = points[(i + j - 1) % params.order]
            assert(affine_expected2 == points[(i + j - 1) % params.order])
...
def test_ddbl_formula(formula, params):
    # Kummer-line has elements in the projective space P^2
    assert( len(params.curve.coordinate_model.variables) == 2 )
    affine_gen = kummer_to_affine(params.generator, params)
...
def main():
    ...
    model_names = {
            "shortw": "Short Weierstrass curve models",
            "montgom": "Montgomery curve models",
            "twisted": "Twisted Edwards curve models",
    ...
    for filename, model in model_list.items():
        curves, length = json2curves("data/" + filename + ".json")
        print("="*len(model_names[filename]) + "\n" + model_names[filename])
        # ===========================================================================================================================
        for i in range(0, length, 1):
            message = "PROCESSING: " + curves['curves'][i]['desc']
            print("-" * len(message) + "\n" + message)
            with open('%s.log' % curves['curves'][i]['name'], 'w') as f:
                        ...
                        if (formula.shortname in tests) or (formula.shortname in tests_kummer):
                            try:
                                if len(coords.variables) > 2:
                                    ...
                                    res = tests[formula.shortname](formula, curve)
                                else:
                                    # Kummer line: differential point additions and doubling
                                    assert( len(coords.variables) == 2)
                                    res = tests_kummer[formula.shortname](formula, curve)
